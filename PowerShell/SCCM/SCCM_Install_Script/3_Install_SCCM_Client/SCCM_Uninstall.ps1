...
foreach($server in $servers){
    ...
    if( Test-Connection $server -Count 1 -Quiet){
        ...
        try{
                ...
                #Start-Process -FilePath XCopy -ArgumentList "$sccmClientInstaller $dest /E /C /I /Q /H /R /Y" -Wait
                ##Copy-Item $sccmClientInstaller -Destination \\$s\c$\Temp\Client -Recurse  -force
                
                Write-Host "`t $s `t Uninstall in progress"
                Start-Process -FilePath .\PsExec.exe -ArgumentList "\\$s cmd /c C:\Windows\ccmsetup\ccmsetup.exe /Uninstall"
