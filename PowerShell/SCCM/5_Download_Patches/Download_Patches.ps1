$packageName = "Package-Apr-2018"
write-host "Generating package list. Please wait..." -ForegroundColor Yellow
$packages = Get-CMSoftwareUpdateDeploymentPackage | select -ExpandProperty name
$checkPkg = ""
do{
    cls
    Write-Host "Select Package to download patches" -ForegroundColor Yellow
    $packages
    $packageName = Read-Host "Select Pacakge Name"
    $checkPkg = Get-CMSoftwareUpdateDeploymentPackage -Name $packageName
}while($checkPkg -eq $null)
...
foreach($patch in $patches){
    ...
    if($softwareUpdates.count -lt 1){
                ...
                Write-Host "$patch Not Required. Now downloading all available. Count is $($softwareUpdates.count)" -ForegroundColor Red
